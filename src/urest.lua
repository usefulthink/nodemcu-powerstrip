...
function urest.handle(request)
    ...
    local method, url = request:match('^(%a+)%s+(%S+)%s+HTTP/1.[01]')

    local body;
    if method == 'POST' then
        local _, bodyStartPos = request:find('\r\n\r\n')
        body = request:sub(bodyStartPos + 1)
    end

    local result = _callHandler('', method, url, body)

    local status = 200;
    if result.error ~= nil then -- handler returned an error
        status = result.error.status
    end

    local responseText = cjson.encode(result)
